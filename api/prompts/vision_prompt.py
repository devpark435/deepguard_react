...
### 🍕 음식·정물 사진일 때
**⚠ 음식·정물 사진에는 균형 탐지 원칙 대신 민감도 우선 원칙을 적용하세요.**
AI 생성 음식 이미지는 "광고보다 더 완벽한 광고 사진"처럼 보이는 것 자체가 핵심 시그니처입니다. 아래 항목에서 "약간이라도 너무 완벽하다"는 느낌이 들면 detected로 기록하세요.

- **재료의 획일성 (최우선)**: 딸기·블루베리·체리 등 과일이 모두 동일한 크기·색·형태인가? 실제 사진에선 반드시 크기·색·형태 편차가 있음. 균일하면 즉시 detected
- **식감 질감**: 빵·고기·케이크 등의 결(grain/texture)이 반복 패턴처럼 균일하거나 렌더링된 느낌인가?
- **수분·광택**: 식재료의 광택이 물리적 반사가 아닌 렌더링된 specular highlight처럼 보이는가?
- **도마·그릇 표면**: 흠집·사용감이 없이 지나치게 깨끗하거나, 나무 결이 규칙적인가?
- **배경 소품 배치**: 꽃·그릇·천 등 소품이 "포토스튜디오 세팅"처럼 과도하게 완벽하게 배치되어 있는가?
- **조명 균일성**: 음식 전체가 고르게 빛나고 그림자가 없거나 지나치게 부드러운가?
→ 위 항목들은 주로 `background_distortion`(재료 획일성·소품 배치), `over_smoothing`(질감 균일), `lighting_inconsistency`(조명 완벽함)으로 기록
