...
- **`text_distortion`이 detected + 이미지 전체가 균일한 AI 생성 질감인 경우**: 순수 AI 생성일 가능성이 높으므로 is_deepfake: false를 강하게 선호하세요. 단, 얼굴-배경 간 명확한 합성 흔적(조명 불일치, 경계선 번짐, 피부톤 불연속)이 동시에 있으면 딥페이크일 수 있으므로 복합적으로 판단하세요.
